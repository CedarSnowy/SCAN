...
if __name__ == '__main__':
    ...
    n_hop = 2
    n_max = 100
    save_path = f'./models/adam/batch-{batch_size}_nhop-{n_hop}_nmax-{n_max}/'
    ...
    opt_dataset_train = {
                        ...
                        "dialog_samples": "./dataset/dialog_samples_list.pkl", 
    ...
    data = AttnIODataset(opt=opt_dataset_train,transform=transforms.Compose([ToTensor(opt_dataset_train)]))
    ...
    opt_model = {"n_entity_seen": len(data.entity2entityID_seen), 
                ...
                "epoch": 20, "n_hop": n_hop, "n_max": n_max,"model_directory":save_path, "model_name": f'AttnIO', "clip": 5, "self_loop_id": data.relation2relationID["self loop"]}
    ...
    valid_seen_indices = list(range(10583, 10583+600))
    valid_unseen_indices = list(range(10583+600,10583+1200))
    test_indices = list(range(len(data)-1200, len(data)))

    # 使用 Subset 创建划分的子数据集
    train_data = Subset(data, train_indices)
    valid_seen_data = Subset(data, valid_seen_indices)
    valid_unseen_data = Subset(data,valid_unseen_indices)
    test_data = Subset(data, test_indices)

    # 使用Dataloader加载
    train_dataloader = DataLoader(train_data, batch_size=batch_size, shuffle=True,collate_fn=AttnIO_collate)
    valid_seen_dataloader = DataLoader(valid_seen_data, batch_size=batch_size,collate_fn=AttnIO_collate)
    valid_unseen_dataloader = DataLoader(valid_unseen_data, batch_size=batch_size,collate_fn=AttnIO_collate)
    #test_dataloader = DataLoader(test_data, batch_size=batch_size,collate_fn=AttnIO_collate)

    
    model.train_model(train_dataloader,valid_seen_dataloader,valid_unseen_dataloader)
