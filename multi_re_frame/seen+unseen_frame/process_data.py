...
def save_kg(file_path):
    unseen_entity = get_unseen_entity(data_path + 'data.json')

    kg_whole = get_whole_kg(file_path)
    #kg_whole = get_kg_from_triple(file_path)
    ...
    all_entity = seen_entity + unseen_entity
    ...
    with open(save_path + 'kg_whole.pkl','wb') as file:
        pickle.dump(kg_whole,file)

    with open(save_path +"kg_seen.pkl", "wb") as file:
        pickle.dump(kg_seen, file)

    with open(save_path +"kg_unseen.pkl", "wb") as file:
        pickle.dump(kg_unseen, file)

    return kg_whole,seen_entity, unseen_entity,all_entity
...
def index_and_embedding_entity(seen_entity, unseen_entity):
    ...
    with open(save_path +"entity2entityID_seen.pkl", "wb") as file:
        print(f'save{file}')
        pickle.dump(entity2entityID_seen, file)

    with open(save_path +"entityID2entity_seen.pkl", "wb") as file:
        pickle.dump(entityID2entity_seen, file)

    with open(save_path +"entity2entityID_unseen.pkl", "wb") as file:
        pickle.dump(entity2entityID_unseen, file)

    with open(save_path +"entityID2entity_unseen.pkl", "wb") as file:
    ...
    with open(save_path +"entity_embeddings_seen.pkl", "wb") as file:
        pickle.dump(entity_embeddings_seen, file)

    with open(save_path +"entity_embeddings_unseen.pkl", "wb") as file:
        pickle.dump(entity_embeddings_unseen, file)

def index_and_embedding_relation(kg_whole):
    #kg_whole = get_whole_kg(data_path + 'data.json')
    kg_whole = get_kg_from_triple(data_path + 'opendialkg_triples.txt')

    idx, relation2relationID = 1, {}
    for values in kg_whole.values():
        for triple in values:
            rel = triple[1]
            #print(rel)
            if rel not in relation2relationID.keys():
                relation2relationID[rel] = idx
                idx += 1
    ...
    with open(save_path +"relation2relationID.pkl", "wb") as f:
        pickle.dump(relation2relationID, f)
    
    with open(save_path +"relationID2relation.pkl", "wb") as f:
        pickle.dump(relationID2relation, f)

    with open(save_path +"relation_embeddings.pkl", "wb") as f:
        pickle.dump(relation_embeddings, f)

def process_dialog(file_path,seen_percentage,max_history_num):
    # 读取data.json，跳过第一行
    ...
    prefix_size = 1
    rules = pd.read_csv('../data/rules_exact.csv')
    ...
    for line in tqdm(lines):
        ...
        for i in range(len(dialogs)):
            ...
            if path_num > 0:
                ...
                entity_history.append(path[0][0])
                ...
                extra_utterances = utterances[len(last_utterances):]

                if len(last_utterances):
                    one_sample['extra_utterances'] = extra_utterances
                
                last_utterances = copy.deepcopy(utterances)
                ...
                one_sample['paths history'] = copy.deepcopy(paths_history)

                jump_rels = []
                for path_history in paths_history:
                    one_jump_rel = []
                    for triple in path_history:
                        one_jump_rel.append(triple[1])
                    jump_rels.append(one_jump_rel)


                has_prefix = len(jump_rels)

                #根据前缀关系进行查找当前可能关系
                if has_prefix:
                    key_prefix_rels = jump_rels[prefix_size:]
                    search_rels = predict_rel(key_prefix_rels,rules)
                    search_rels = list(itertools.chain.from_iterable(search_rels))
                    search_rels = list(set(list(itertools.chain.from_iterable(search_rels))))
                    search_rels_index =[relation2relationID[x] for x in search_rels]
                
                    one_sample['search rels index'] = search_rels_index

                #print(paths_history)
                paths_history.append(path)
                #one_sample['entity history'] = copy.deepcopy(entity_history)
    ...
    for dialogID,samples_flag in dialog_samples.items():
        ...
        if flag == 0:
            for sample in samples:
                ...
                # for phrase in start_entity:
                #     masked_sentence = find_matching_word(phrase,masked_sentence)
...
def process_seen_entity(utterances,sample):
    all_context = ' '.join(utterances)
    encoder_utterances = get_albert_representations(all_context)
    sample['utterances'] = encoder_utterances

    if 'extra_utterances' in sample.keys():
        sample['extra_utterances'] = get_albert_representations(' '.join(sample['extra_utterances']))
    return sample


def process_unseen_entity(start_entity,utterances,sample):
    utterances_copy = copy.deepcopy(utterances)
    for phrase in start_entity:
        for idx,utterance in enumerate(utterances):
            pattern = r"\b" + re.escape(phrase) + r"\b"  # 使用正则表达式匹配整个单词
            utterances[idx] = re.sub(pattern, "[MASK]", utterance, flags=re.IGNORECASE)
    ...
    if len(mask_embeddings):
        avg_mask_embedding = torch.stack(mask_embeddings)
        avg_mask_embedding = torch.mean(avg_mask_embedding,dim = 0)
        sample['MASK embedding'] = [avg_mask_embedding]
    else:
        sample['MASK embedding'] = []

    all_context = ' '.join(utterances_copy)
...
def encoder_dialog(dialog_samples,seen_percentage,unseen_entity): 
    dialog_samples_list = []

    for dialogID,samples_flag in tqdm(dialog_samples.items()):
        samples = samples_flag['samples']

        if samples_flag['state'] == 'train':           
            for i in range(len(samples)):
                sample = samples[i]
                utterances = sample['utterances']
                start_entity = sample['starts'][0]

                # 随机抽取一部分subject当作unseen
                if random.random() <= seen_percentage:
                    samples[i] = process_seen_entity(utterances,sample)
                else:
                    sample['train-unseen'] = 1
                    samples[i] = process_unseen_entity(start_entity,utterances,sample)
    
            samples_flag['samples'] = samples
            
        elif samples_flag['state'] == 'valid' or samples_flag['state'] == 'test':
            for i in range(len(samples)):
                sample = samples[i]
                utterances = sample['utterances']
                start_entity = sample['starts'][0]

                if start_entity not in unseen_entity:
                    samples[i] = process_seen_entity(utterances,sample)
                else:
                    samples[i] = process_unseen_entity(start_entity,utterances,sample)
    ...
    with open(save_path +'dialog_samples_list.pkl','wb') as f:
        pickle.dump(dialog_samples_list, f)


def multi_apperance(dialog_samples):
    count = 0
    multi_apper_list = []
    for dialogID_samples_flag in dialog_samples:
        dialogID = dialogID_samples_flag['dialogID']
        samples_flag = dialogID_samples_flag['samples_flag']

        if samples_flag['state'] != 'test':
            continue
        ...
        for sample in samples:
            ...
            starts.append(start_entity[0])

        if len(starts) != len(set(starts)):
            count += 1
            multi_apper_list.append({'dialogID':dialogID,'samples_flag':samples_flag})

    with open(save_path +'multi_apper_test_list.pkl','wb') as f:
        pickle.dump(multi_apper_list, f)

    return multi_apper_list
    


if __name__ == "__main__":

    kg_whole, seen_entity, unseen_entity,all_entity = save_kg(data_path + 'data.json')

    #index_and_embedding_all_entity(all_entity)

    #index_and_embedding_entity(seen_entity, unseen_entity)

    #index_and_embedding_relation(kg_whole)

    dialog_samples = process_dialog(data_path + 'coref_data.json',0.9,3)

    temp_save_path = './dataset/dialog_samples.pkl'

    with open(temp_save_path, "wb") as file:
        pickle.dump(dialog_samples, file)

    #mulit_apper_list = multi_apperance(dialog_samples)
   
    encoder_dialog(dialog_samples,0.9,unseen_entity)
