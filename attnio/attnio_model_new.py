...
class AttnIO(nn.Module):
    def __init__(self, in_feats, out_feats, num_heads, entity_seen_embeddings, entity_unseen_embeddings,relation_embeddings, self_loop_id, device):
        ...
        # Embeddings
        self.entity_seen_embeddings = entity_seen_embeddings
        self.entity_unseen_embeddings = entity_unseen_embeddings
    ...
    #调用这里
    #传一个embedding的list
    def forward(self, graph, seed_set, dialogue_context,state,flag):

        if state == 'test' and flag == 0:
            feat = self.entity_unseen_embeddings(graph.ndata["nodeId"]) 
            # print('fing unseen embedding')
        else:
            feat = self.entity_seen_embeddings(graph.ndata["nodeId"]) 
