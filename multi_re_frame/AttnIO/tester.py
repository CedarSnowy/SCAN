...
from attnio_build import MultiReModel
from dataset_attnio import ToTensor,MultiRe_collate,MultiReDataset
from torch.utils.data import Subset
from dgl.sampling import sample_neighbors
from utils import load_pickle_file
from main import opt_model,opt_dataset
...
def batch_beam_search(model, batch_data, device, topk,recall_entity,recall_path,predict_entity_list,true_entity_list,seen,failure_time):
    with torch.no_grad():
        model.eval()
        ...
        for one_data in batch_data:
            ...
            one_successfully_sample = one_data['one_successfully_sample']
            ...
            for i in range(sample_num):
                ...
                successfully_sample = one_successfully_sample[i]['successfully_sample']


                if len(seed_entities) == 0:
                    continue
                elif not successfully_sample:
                    failure_time += 1
                    ...
                    updated_subgraph, expilcit_entity_rep = model(state,dialogue_representation, extra_dialogue_representation,seed_entities, subgraph, sub_embedding,entity_state_1_jump,
    ...
    with torch.no_grad():
        for ks in K:
            ...
            for batch in tqdm(ConvKGDatasetLoaderTest):
                ...
                batch_beam_search(model, batch, opt["device"], ks,[recall_entity_one_jump,recall_entity_two_jump,recall_entity_all],[recall_path_one_jump,recall_path_two_jump,recall_path_all],predict_entity_list,true_entity_list,seen,failure_time)
...
if __name__ == '__main__':
    ...
    result_name = f'./result/big-graph/dense_no_inputGT'
    result_file = result_name+'.csv'

    opt_dataset['do_predict'] = do_predict
    opt_dataset['batch'] = batch_size
    opt_dataset['share_subgraph'] = share_subgraph
    ...
    #                     'entity_embeddings_all':data_directory + 'entity_embeddings_all.pkl',
    ...
    #                     "device": device,
    #                     "n_hop": n_hop, "n_max":n_max, "max_dialogue_history": 3,'batch':batch_size,'seen_percentage':0.95,'train_unseen_rate':0.95,'max_edge':10000,'prefix_size':1,'do_predict':do_predict}
    ...
    for i in range(2,22,2):
        model_path = f'./models/attnio_bigGraph/batch-4/'
