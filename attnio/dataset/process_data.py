...
def process_test(dialog_samples):
    test_count = 0
    for dialogID,samples_flag in dialog_samples.items():
        flag,state = samples_flag['flag'],samples_flag['state']
        ...
        if state == 'test' and flag == 0:
            for sample in samples:
                ...
                phrase_pos = masked_sentence.lower().find(start_entity.lower())  # 查找词组在句子中的位置
                if phrase_pos != -1:
                    sample['MASK embedding'] = 1
                    test_count += 1
                else:
                    sample['MASK embedding'] = 0
...
if __name__ == "__main__":
    ...
    dialog_samples = process_dialog(file_path,1,3)
    
    process_test(dialog_samples)
