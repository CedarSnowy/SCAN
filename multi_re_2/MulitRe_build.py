...
class MultiReModel(nn.Module):
    def __init__(self, opt):
        ...
        for param in self.attnIO.parameters():
            param.requires_grad = True
    ...
    def forward(self, dialogue_representation, seed_entities, subgraph,sample_mask,node2nodeId,flag,state,startEntity_in_seenKG, last_graph,last_entity_rep):
        if  (state == 'train' and flag == 0) or (state == 'test' and flag == 0) or (state == 'valid' and not startEntity_in_seenKG):
            ...
            subgraph = self.attnIO(graph = subgraph, seed_set = seed_entities, dialogue_context = dialogue_representation,unseen_embeddings = new_ndata,_new = 1,last_graph = last_graph,last_entity_rep = last_entity_rep)
        else:
            #print(subgraph.ndata)
            subgraph = self.attnIO(graph = subgraph, seed_set = seed_entities, dialogue_context = dialogue_representation,unseen_embeddings = None, _new = 0, last_graph = last_graph, last_entity_rep = last_entity_rep)
        return subgraph
    ...
    def process_batch(self, batch_data,train):     
        ...
        for idx,one_batch_data in enumerate(batch_data):
            one_dialog_loss = []
            ...
            for i in range(sample_num):
                ...
                if len(seed_entities) == 0:
                    continue
                if ((state == 'train' and flag == 0) or (state == 'test' and flag == 0) or (state == 'valid' and not startEntity_in_seenKG)) and len(sample_mask) == 0: # 句子中没匹配上MASK，得不到对应MASK位置的encoder
                    continue
                else:
                    updated_subgraphs = []
                    ...
                    updated_subgraph, expilcit_entity_rep = self(dialogue_representation, seed_entities, subgraph,sample_mask,node2nodeID,flag,state,startEntity_in_seenKG, last_graph,last_entity_rep)
