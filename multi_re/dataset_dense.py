...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if (flag == 1 and state == 'train') or (flag == 0 and state =='train') or (flag == 1 and state == 'test') : # AttnIO
            successors_list += self.graph_seen.successors(seed_entities[0]).tolist()
            subgraph = dgl.node_subgraph(self.graph_seen,successors_list)
            ...
            successors_list += self.graph_unseen.successors(seed_entities[0]).tolist()
            subgraph = dgl.node_subgraph(self.graph_unseen,successors_list)
            ...
            if startEntity_in_seenKG:
                ...
                head_node,tail_node = subgraph.edges()[0],subgraph.edges()[1]
                for i in range(len(head_node)):
                    head_entities.append(node_new2old[head_node[i]])
                    tail_entities.append(node_new2old[tail_node[i]])
                edge_relations.extend(subgraph.edata['edge_type'].tolist())
            else:
                successors_list += self.graph_unseen.successors(seed_entities[0]).tolist()
                subgraph = dgl.node_subgraph(self.graph_unseen,successors_list)
