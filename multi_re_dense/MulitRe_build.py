...
class MultiReModel(nn.Module):
    def __init__(self, opt):
        ...
        self.earl = Earl(self.in_dim,self.entity_emb_seen,opt["relation_embeddings"],self.device)
    ...
    def forward(self, dialogue_representation, seed_entities, subgraph,sub_embedding,node2nodeId, nodeID2node, flag,state,startEntity_in_seenKG, last_graph,last_entity_rep,entity_state):
        graph = deepcopy(subgraph)
        entity_embeddings = self.earl(graph = graph,encoder_output = dialogue_representation, seed_entities = seed_entities,sample_mask = sub_embedding,node2nodeId = node2nodeId, nodeID2node = nodeID2node, entity_state = entity_state, state = state)
        ...
        for idx,one_batch_data in enumerate(batch_data):
            ...
            for i in range(sample_num):
                ...
                if not startEntity_in_seenKG and len(sub_embedding) == 0: # 句子中没匹配上MASK，得不到对应MASK位置的encoder
                    ...
                    updated_subgraph, expilcit_entity_rep = self(dialogue_representation, seed_entities, subgraph, sub_embedding, node2nodeID, nodeID2node, flag, state,startEntity_in_seenKG, last_graph, last_entity_rep, entity_state)
